...
const { mapDBToSongModel, mapDBToSongListModel } = require("../../utils");
...
class SongsService {
  ...
  async getSongs(title = "", performer = "") {
    ...
    if (title) {
      conditions.push(`LOWER(title) LIKE $${paramCount}`);
      values.push(`%${title.toLowerCase()}%`);
      paramCount++;
    }

    if (performer) {
      conditions.push(`LOWER(performer) LIKE $${paramCount}`);
      values.push(`%${performer.toLowerCase()}%`);
      paramCount++;
    }

    if (conditions.length > 0) {
      query.text += " AND " + conditions.join(" AND ");
      query.values = values;
    }
